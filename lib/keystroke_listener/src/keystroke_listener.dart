...
class _KeystrokeListenerState extends State<KeystrokeListener> {
  ...
  KeyEventResult _handleKeyEvent(FocusNode node, KeyEvent event) {
    ...
    if (widget.enableVisualDebug) {
      // Show snackbar with pressed key (best-effort, no crash if no Scaffold).
      try {
        ScaffoldMessenger.of(context)
          ..clearSnackBars()
          ..showSnackBar(SnackBar(content: Text('Pressed: ${event.logicalKey.keyLabel}')));
      } catch (_) {
        // No Scaffold available — visual debug is best-effort only.
      }
    }
