...
class _SBannerState extends State<SBanner> {
  ...
  void didUpdateWidget(covariant SBanner oldWidget) {
    ...
    // Do NOT reset when the widget's child instance identity changes alone —
    // that causes spurious re-measure and visual flicker in common parent rebuilds.
    // Keep reset for circular mode and explicit borderRadius changes only.
  ...
  Widget build(BuildContext context) {
    ...
    if (_childSize == null && widget.isChildCircular) {
      return Stack(
        children: [
          measuredChild,
          Offstage(child: widget.bannerContent),
        ],
      );
    }
    ...
    if (widget.onTap != null) {
      bannerWidget = GestureDetector(onTap: widget.onTap, child: bannerWidget);
    }
